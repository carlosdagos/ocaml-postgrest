...
let combine_with logical_operator a b =
  ...
  let combine_tuple a' b' =
    if a'.negated_comb = b'.negated_comb && a'.op_combined = b'.op_combined
    then `Combined { negated_comb = a'.negated_comb;
  ...
  match (a, b) with
    (`Combined a', `Combined b') ->
     combine_tuple a' b'
  | (`Combined a', _          )  ->
     if a'.op_combined = logical_operator
     then combine_into a' b
     else combine_simple a b
  | (_          , `Combined b')  ->
     if b'.op_combined = logical_operator
     then combine_into b' a
     else combine_simple b a
...
(* Negate a condition *)
let not' cond = match cond with
    `Boolean b -> `Boolean (not b)
  | `Operator o ->
     let updated = { o with negated_op = (not o.negated_op); }
     in `Operator updated
  | `Combined c ->
     let updated = { c with negated_comb = (not c.negated_comb); }
     in `Combined updated
...
let string_of_value_list (v : value_t list) =
  let concmap = B.(Fn.compose
                     (String.concat ~sep:",")
                     (List.map ~f:string_of_value))
  in
  B.Printf.sprintf "(%s)" (concmap v)

let rec string_of_condition ?(tl=true) c = match c with
    `Boolean b  -> string_of_bool b
  | `Operator o ->
     let negated_str  = if o.negated_op then "not." else "" in
     let operator_str = string_of_operator o.op in
     let fmt = B.Printf.(if tl
     ...
     in fmt
          ...
          negated_str
          operator_str
          (string_of_value o.value)
  | `Combined c ->
     let negated_str  = if c.negated_comb then "not." else "" in
...
let makeCond op attribute value : condition =
  let op_cond = { negated_op = false; op; attribute; value; }
  in `Operator op_cond

let eq'        = makeCond `Eq
let neq' a     = B.Fn.(compose not' (eq' a))

let gt' a (v : numeric_value_t)   = makeCond `Gt a (v :> value_t)
let gte' a (v : numeric_value_t)  = makeCond `Gte a (v :> value_t)
let lt' a (v : numeric_value_t)   = makeCond `Lt a (v :> value_t)
let lte' a (v : numeric_value_t)  = makeCond `Lte a (v :> value_t)

let is' a (v : simple_value_t)    = makeCond `Is a (v :> value_t)

let like' a (v : string_value_t)  = makeCond `Like a (v :> value_t)
let ilike' a (v : string_value_t) = makeCond `ILike a (v :> value_t)
let fts' ?lang  a (v : string_value_t) =
  makeCond (`Fts lang) a (v :> value_t)
let plfts ?lang a (v : string_value_t) =
  makeCond (`Plfts lang) a (v :> value_t)
let phfts ?lang a (v : string_value_t) =
  makeCond (`Phfts lang) a (v :> value_t)

let stringIn' a ( v : string_value_t list) =
  `Stringlit (string_of_value_list (v :> value_t list)) |>
    makeCond `In a

let numberIn' a ( v : numeric_value_t list) =
  `Stringlit (string_of_value_list (v :> value_t list)) |>
    makeCond `In a
